...
void main() {
  ...
  testWidgets('renders the provided child', (tester) async {
    await pumpWidget(tester, const ShakeWidget(trigger: 0, child: Text('Shake Me')));

    expect(find.byType(ShakeWidget), findsOneWidget);
    expect(find.text('Shake Me'), findsOneWidget);
  });

  testWidgets('triggers the shake animation when trigger value changes', (tester) async {
    await pumpWidget(tester, const ShakeWidget(trigger: 0, child: Text('Shake Me')));

    // The AnimatedBuilder is present within the ShakeWidget.
    expect(
      find.descendant(of: find.byType(ShakeWidget), matching: find.byType(AnimatedBuilder)),
      findsOneWidget,
    );

    // Rebuild with a new trigger value — didUpdateWidget calls forward(from: 0).
    await pumpWidget(tester, const ShakeWidget(trigger: 1, child: Text('Shake Me')));
  ...
  testWidgets('does not re-trigger when trigger stays the same', (tester) async {
    await pumpWidget(tester, const ShakeWidget(trigger: 5, child: Text('Shake Me')));
